...
module ParserUtil =
    ...
    let parseString (text: string) =
        let lexbuf =
            LexBuffer<_>.FromBytes (Encoding.UTF8.GetBytes(text))
    ...
    let parseFromFile filename =
        if File.Exists("./input/" + filename) then
            parseString (File.ReadAllText("./input/" + filename))
        else
...
module CompilerUtil =
    ...
    let goOpt p =
        run (code2ints (CodeGenerationOpt.CP p))
    ...
    let goTrace p =
        runTrace (code2ints (CodeGeneration.CP p))
